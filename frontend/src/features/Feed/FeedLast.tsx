...
function FeedLast() {
  ...
  const handleClickAtmos = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setAtmos(atmos.map((v, i) => (i === index ? { ...v, check: !v.check } : v)));
  };
        ...
        if (result?.status === 409) {
          toast.error(
            <div style={{ width: 'inherit', fontSize: '14px' }}>
              해당 위치(좌표)에 등록된 장소가 있습니다. 위치를 조정해주세요
            </div>,
            {
              position: toast.POSITION.TOP_CENTER,
              role: 'alert',
  ...
  return (
    <div>
      {feedstore.type === true ? (
        ...
        <div>
          ...
          >
            ...
            >
              {placeAddres.length > 0 ? placeAddres : '주소를 검색해주세요'}
            </button>
          </div>
        </div>
      )}
      ...
      >
        ...
        <h1 style={{ width: 280, fontSize: 16, fontWeight: 700, fontFamily: 'NotoSansKR', marginBottom: 0 }}>평점</h1>
        <span
          style={{
            position: 'relative',
            fontSize: 38,
