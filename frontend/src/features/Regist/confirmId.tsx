...
function ConfirmId({progress} : MyProps){
    ...
    const handleCheckDuplicationId = (e : React.MouseEvent<HTMLButtonElement>) =>{
        ...
        e.preventDefault();
        toast.success(
            <div style={{ width: "inherit", fontSize : "14px" }}>사용가능한 아이디 입니다.</div>,
                {
        ...
        <main>
            <article>
                <div>
                    ...
                    <Title>아이디 설정</Title>
                    <Input  onChange={handleCheckId}
                            placeholder='아이디를 입력해주세요.'
                            />
                    <ConfirmBtn onClick={handleCheckDuplicationId} disabled={!isId}>중복확인</ConfirmBtn>
